 ...
 * Date: April 18, 2024
 * This class manages all the functions on the Equipment page.
 * Users can create the new Equipment table, read table data, and catch sql exceptions.
*/
	...
	public class EquipmentManagerUI
	{
		...
		public void CreateEquipmentList()
		{
			...
			if (result == 0) {
				...
				{
					Console.WriteLine("Error: SQL syntax error.", e.ToString());
				}
			}
			connection.Close();
		}
		...
		public List<Equipment> ReadEquipment()
		{
			...
			while (reader.Read())
			{
				...
				{
					Console.WriteLine("Error: SQL data reader error.", e.ToString());
				}
